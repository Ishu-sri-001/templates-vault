/**
 * Vendored from `virtual-scroll` v2.2.1 (src/index.js, src/support.js,
 * src/keycodes.js), with its `tiny-emitter` dependency inlined.
 *
 * Ported to TypeScript; behaviour is unchanged from upstream. Kept here so the
 * smooth-carousel effect ships every file it needs and pulls no npm packages.
 *
 * The MIT License (MIT) — Copyright (c) 2014 Florian Morel
...
/** IE10's touch-action property, absent from lib.dom's CSSStyleDeclaration. */
...
/** Inlined `tiny-emitter`. `.e` is read by `on()`/`off()` to decide when to
 *  bind and unbind the DOM listeners, so it stays part of the shape. */
...
export interface VirtualScrollEvent {
  ...
  deltaY: number;
  originalEvent: Event;
  /** Never set by this library — upstream `smooothy` reads it defensively. */
  touchDevice?: boolean;
}
