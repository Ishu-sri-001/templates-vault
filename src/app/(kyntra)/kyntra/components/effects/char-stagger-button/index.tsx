...
export interface CharStaggerButtonOwnProps {
  ...
  staggerStep?: number;
  showLine?: boolean;
  /** Underline the arrow as well */
  lineSpansArrow?: boolean;
...
export type CharStaggerButtonProps = CharStaggerButtonOwnProps &
  Omit<
    ComponentPropsWithoutRef<typeof Link>,
    "href" | "onClick" | keyof CharStaggerButtonOwnProps
  >;
...
export default function CharStaggerButton({
  ...
  useEffect(() => {
    ...
    [...sourceText].forEach((char, index) => {
      ...
      span.style.transitionDelay = prefersReducedMotion
        ? "0s"
        : `${index * staggerStep}s`;
  ...
  return (
    <Link
      ...
      style={mergedStyle}
      className={`inline-flex items-center gap-2 mt-[0.3vw] no-underline transition-colors duration-500 ease-[cubic-bezier(0.625,0.05,0,1)] motion-reduce:transition-none max-[1025px]:motion-reduce:transition-none max-md:motion-reduce:transition-none ${
        showLine && lineSpansArrow ? "relative" : ""
      } ${className}`.trim()}
    >
      <div>
        <span
        ...
        >
          <span
            ...
            className={`overflow-hidden relative inline-block leading-[1.2] max-[1025px]:text-[3vw]  ${textClassName}`.trim()}
          >
            {sourceText}
          </span>

          {showLine && !lineSpansArrow && lineElement}
        </span>
      </div>

      {showArrow && Icon && (
        <div
          className={`flex size-[1.18em] items-center justify-start overflow-hidden ${iconClassName}`.trim()}
        >
          {iconVariant === "single" ? (
