...
const DEFAULT_HOVER_COLOR = "#111111";
const DEFAULT_LINE_HEIGHT = "1px";
const DEFAULT_LINE_BOTTOM = "15%";
const CHAR_TRANSLATE_Y = "1.3em";

export interface CharStaggerButtonOwnProps {
  text?: string;
  href?: string;
  className?: string;
  ...
  /** Thickness of the hover underline - any CSS length (e.g. "3px", "0.2vw"). */
  lineHeight?: string | number;
  /** How far the hover underline sits above the text box's bottom edge. */
  lineBottom?: string | number;
  hoverColor?: string;
  showArrow?: boolean;
...
export type CharStaggerButtonProps = CharStaggerButtonOwnProps & Omit<ComponentPropsWithoutRef<typeof Link>, 'href' | 'onClick' | keyof CharStaggerButtonOwnProps>;

export default function CharStaggerButton({
  text = DEFAULT_TEXT,
  href = DEFAULT_HREF,
  className = "",
  textClassName = "",
  ...
  lineBottom = DEFAULT_LINE_BOTTOM,
  ...
  return (
    <Link
      ...
      style={mergedStyle}
      className={`inline-flex items-center gap-2 no-underline transition-colors duration-500 ease-[cubic-bezier(0.625,0.05,0,1)] motion-reduce:transition-none max-[1025px]:motion-reduce:transition-none max-md:motion-reduce:transition-none ${className}`.trim()}
    >
      ...
      {showArrow && Icon && (
        <div className={`relative flex size-[1.18em] items-center justify-start overflow-hidden ${iconClassName}`.trim()}>
          {iconVariant === "single" ? (
            ...
            <>
              {/* The one already in flow - slides out to the right on
                  hover, handing off to the absolute one below sliding in
                  from the same off-screen position. */}
              <Icon
                className={`size-[1.18em] shrink-0 transition-transform duration-500 ease-[cubic-bezier(0.625,0.05,0,1)] motion-reduce:transition-none max-[1025px]:motion-reduce:transition-none max-md:motion-reduce:transition-none ${
                  isActive && !prefersReducedMotion ? "translate-x-full" : "translate-x-0"
                }`}
              />
              <Icon
                aria-hidden="true"
                className={`absolute inset-0 size-[1.18em] shrink-0 transition-transform duration-500 ease-[cubic-bezier(0.625,0.05,0,1)] motion-reduce:transition-none max-[1025px]:motion-reduce:transition-none max-md:motion-reduce:transition-none ${
                  isActive && !prefersReducedMotion ? "translate-x-0" : "-translate-x-full"
                }`}
              />
            </>
          )}
